...
export async function getMostReviewedBook(connectionType: ConnectionTypes) {
  try {
    if (connectionType === "kai") {
      return (
        await getEleganceClient("kai").requests.query<Book[]>({
          collection: "books",
          pipeline: [{ $sort: { reviews: -1, title: 1 } }, { $limit: 1 }]
        })
      )[0];
    } else {
      const book = (
        await getEleganceClient("mysql").requests.query<MySQLBook[]>({
          query: `SELECT * FROM books_chat_mysql.books ORDER BY reviews DESC, title ASC LIMIT 1`
        })
      )[0];
